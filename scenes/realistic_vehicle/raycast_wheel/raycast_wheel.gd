...
#Third-Party Code from VitaVehicle
...
func _physics_process(_delta):
	...
	if Steer and abs(car.steer)>0:
		...
		global_transform = lasttransform
		rotate_object_local(Vector3(0,1,0),deg_to_rad(90.0))
		var roter_estimateed = deg_to_rad(global_rotation.y)
...
#			skvol *= skvol
	...
	var orth_basis_vel = $Velocity2.global_transform.basis.orthonormalized()
	
	var forces = orth_basis_vel * Vector3(0,0,1) * directional_force.z + orth_basis_vel * Vector3(1,0,0) * directional_force.x + orth_basis_vel * Vector3(0,1,0) * directional_force.y
